...
function normalizeCapability(capability, serviceId, index, allowInsecureHttp, environment) {
  ...
  if (!['browser', 'http'].includes(transport)) throw new Error(`${prefix}.transport must be browser or http`);
...
function normalizeConnectorService(service, index, allowInsecureHttp, environment) {
  ...
  if (!Array.isArray(origins) || origins.length === 0 || origins.length > 32) {
    throw new Error(`${prefix}.origins must contain 1 to 32 exact origins`);
  }
