...
test('central connector config rejects the removed desktop topology', async (t) => {
  const env = await fixture();
  t.after(() => env.cleanup());
  const { raw, file } = await configFiles(env);
  const cases = [
    [(value) => { value.connectors.desktopWebsocketPath = '/relu/desktop/ws'; }, /desktopWebsocketPath is unsupported/u],
    [(value) => { value.connectors.services[0].clientKinds = ['browser']; }, /clientKinds is unsupported/u],
    [(value) => { value.connectors.services[0].desktopAppIds = ['com.relu.Viewer']; }, /desktopAppIds is unsupported/u],
    [(value) => { value.connectors.services[0].capabilities[0].transport = 'desktop'; }, /transport must be browser or http/u],
  ];
